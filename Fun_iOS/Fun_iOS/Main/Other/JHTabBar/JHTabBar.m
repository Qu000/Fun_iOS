...
- (void)addItems{
    ...
    for (int i=0;i<count;i++){
        JHTabBarItem *item = [[JHTabBarItem alloc] initWithFrame:CGRectMake(i*ScreenW/count, 0, ScreenW/count , 49)];
...
- (void)setBePath:(NSInteger) index{
    //计算中心点
    CGFloat x = ScreenW/count*(index+0.5);
...
- (void)pathAnimation:(NSInteger)formIndex toIndex:(NSInteger)toIndex{
    ...
    //当前中心点
    CGFloat x1 = ScreenW/count*(formIndex+0.5);
    CGPoint center1 = CGPointMake(x1,19);
    //目标中心点
    CGFloat x2 = ScreenW/count*(toIndex+0.5);
    CGPoint center2 = CGPointMake(x2,19);
    /// 相隔几个item
    NSInteger indexPoor = toIndex - formIndex;
    
    // 计算 运动路径长度
    CGFloat length = 4*M_PI*RADIUS+labs(indexPoor)*ScreenW/count;
    //计算一个圆的周长
