...
#define JHGridCount 16
#define JHProgressScale 8

@interface JHBrightnessEchoView : UIView
...
@implementation JHBrightnessEchoView
...
- (void)layoutSubviews
{
    ...
    // 亮度进度条内小格子布局
    CGFloat gridW = (self.brightnessProgressView.bounds.size.width - (JHGridCount + 1)) / JHGridCount;
    CGFloat gridH = 5;
    CGFloat gridY = 1;
    
    for (int i = 0; i < JHGridCount; i++) {
...
// 移动的方向 枚举类型
typedef enum {
    kJHMoveTypePortrait,  // 竖向滑动手势
    kJHMoveTypeLandscape, // 横向滑动手势
    kJHMoveTypeUnknow, // 未知手势
} JHMoveType;

@interface JHBrightnessAndVolumeView ()
...
@implementation JHBrightnessAndVolumeView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        // 初始化音量回显图标
        JHBrightnessEchoView *brightnessEchoView = [JHBrightnessEchoView sharedBrightnessEchoView];
...
- (JHMoveType)judgeMoveType:(UIPanGestureRecognizer *)sender
{
    CGPoint pointT = [sender translationInView:self];
    CGFloat deltaTX = fabs(pointT.x);
    CGFloat deltaTY = fabs(pointT.y);
    if (MAX(deltaTX, deltaTY) < 20) return kJHMoveTypeUnknow;
    if (deltaTX > deltaTY) {
        return kJHMoveTypeLandscape;
    } else return kJHMoveTypePortrait;
}
...
- (void)decideWhatToChange:(UIPanGestureRecognizer *)sender
{
    ...
    if ([self judgeMoveType:sender] == kJHMoveTypePortrait) {
    ...
    } else if ([self judgeMoveType:sender] == kJHMoveTypeLandscape) {
        [self progressChange:sender handle:self.progressChangeHandle];
    } else if ([self judgeMoveType:sender] == kJHMoveTypeUnknow) return;

    if (sender.state == UIGestureRecognizerStateEnded) {
        if ([self judgeMoveType:sender] == kJHMoveTypePortrait) {
            self.progressPortraitEnd();
        }
        if ([self judgeMoveType:sender] == kJHMoveTypeLandscape) {
...
- (void)brightnessChange:(UIPanGestureRecognizer *)sender
{
    ...
    CGFloat delta = -panPoint.y / SCREEN_WIDTH;
...
{
    CGPoint panPoint = [sender translationInView:self.brightnessView];
    CGFloat delta = panPoint.x / JHProgressScale;
    handle(delta);
}
