...
#pragma mark --- 扩展
@interface JHPreviewView : UIView
...
#pragma mark --- 设置frame---layoutSubviews
- (void)layoutSubviews
{
...
@property (nonatomic, strong) JHLoadingView *waitingView;
...
@property (nonatomic, weak) JHBrightnessAndVolumeView *brightnessAndVolumeView;
@property (nonatomic, strong) UITapGestureRecognizer *tapGesture;
@property (nonatomic, strong) UIPanGestureRecognizer *panGuesture;
@property (nonatomic, strong) JHPreviewView *previewView;
@property (nonatomic, strong) NSMutableArray *thumbImages;
@property (nonatomic, strong) AVAssetImageGenerator *imageGenerator;

@end

@implementation JHVideoPlayerView
...
- (JHLoadingView *)waitingView
{
    if (_waitingView == nil) {
        _waitingView = [[JHLoadingView alloc] init];
...
- (JHPreviewView *)previewView
{
    if (_previewView == nil) {
        _previewView = [JHPreviewView sharedPreviewView];
...
- (void)playWithVideoInfo:(VideoItem *)VideoInfo{
    ...
    [defaults setObject:VideoInfo.videoModel.url forKey:@"currentPlayingUrl"];
    
    self.asset = [AVURLAsset assetWithURL:[NSURL URLWithString:VideoInfo.videoModel.url]];
    ...
    dispatch_group_async(group, queue, ^{
        ...
        for (int i = 0; i < captureTimes; i++) {
            UIImage *image = [self thumbImageForVideo:[NSURL URLWithString:VideoInfo.videoModel.url] atTime:10 * i];
            if (image == nil) {
                // 未截取到图片 回退index
                i--;
                continue;
    ...
    dispatch_group_async(group, queue, ^{
        // 添加视频最后一帧缩略图到数组
        UIImage *lastImage = [self thumbImageForVideo:[NSURL URLWithString:VideoInfo.videoModel.url] atTime:self.totalTime];
        if (!lastImage) return;
...
- (IBAction)selectEpisodeAction:(UIButton *)sender {
    ...
    episodeTableView.frame = CGRectMake(0, 0, SCREEN_WIDTH * 0.7, SCREEN_HEIGHT - 100);
